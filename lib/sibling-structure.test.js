...
test('skeleton keeps tags, ids and classes in order and ignores text and other attributes', () => {
  ...
  assert.deepStrictEqual(skeleton(html), ['body', 'main#m.a.b', 'a.c', '/a', 'br', '/main', '/body']);
});

test('nesting differences are structural differences', () => {
  const inside = '<body><a><span>Contact</span></a></body>';
  const after = '<body><a></a><span>Contact</span></body>';
  assert.notStrictEqual(firstDifference(skeleton(inside), skeleton(after)), null);
});
